...
void Adafruit_Thermal::begin(int heatTime) {
  ...
  writeBytes(20);      // Heat interval (500 uS = slower, but darker)
...
#define printDensity   10 // 120% (? can go higher, text is darker but fuzzy)
#define printBreakTime  2 // 500 uS

  writeBytes(18, 35); // DC2 # (print density)
  writeBytes((printBreakTime << 5) | printDensity);

  dotPrintTime = 13000; // See comments near top of file for
  dotFeedTime  =  2400; // an explanation of these values.
  maxChunkHeight = 255;
}
...
int Adafruit_Thermal::getTemperature() {
  writeBytes(27, 118, 2);
